...
public class DistanceTracker : MonoBehaviour
{
    ...
    [Tooltip("Zeitinterval nachdem der Multiplyer bei anhaltendem Kontakt um den MultiplyerAdd hochgez�hlt wird.")]
    ...
    private void FixedUpdate()
    {
        ...
        if (myManager.CurrentGameState == LevelManager.gameState.running)
        {
            ...
            if (timeFromLastContact > breakTime)         // Wenn der letzte Kontakt k�rzer zur�ckliegt als die Breaktime, ist der ContinuouseCounter an - sonst wird er zur�ckgesetzt
            {
                continuouseContactTime = 0;
                timeMultiplyer = 1; // Wieder auf normal zur�ckgesetzt
            }
        }
